...
class YawPlannerNode {
   public:
    YawPlannerNode(ros::NodeHandle& nh) : nh_(nh) {
        ...
        for (int i = 0; i < num_drones_; ++i) {
            ...
            // 新增：发布 yaw（单位：度），话题名 /drone_i_yaw
            std::string yaw_deg_topic = "/drone_" + std::to_string(i) + "_yaw";
            yaw_deg_pubs_.push_back(nh_.advertise<std_msgs::Float64>(yaw_deg_topic, 1));
        }
   ...
   private:
    ...
    void publishYawAndMarkers(std::vector<double>& yaws) {
        ...
        for (int i = 0; i < num_drones_; ++i) {
            ...
            yaw_deg_msg.data = yaws[i] * 180.0 / M_PI;  // rad -> deg
            if (i < (int)yaw_deg_pubs_.size()) {
                yaw_deg_pubs_[i].publish(yaw_deg_msg);
            }
   ...
   private:
    ...
    std::vector<ros::Publisher> yaw_pubs_;      // 弧度制 yaw_cmd
    std::vector<ros::Publisher> yaw_deg_pubs_;  // 度数制 yaw: /drone_i_yaw
