...
double Car_KinoSearch::computeShotTraj(Eigen::Vector3d state1, Eigen::Vector3d state2, std::vector<Eigen::Vector3d>& path_list, double& len, double dt) {
...
int Car_KinoSearch::car_search(Eigen::Vector3d start_state, Eigen::Vector3d end_state) {
    /* ---------- initialize ---------- */
    if (!has_map)
        ROS_ERROR("map is not built for car!");
